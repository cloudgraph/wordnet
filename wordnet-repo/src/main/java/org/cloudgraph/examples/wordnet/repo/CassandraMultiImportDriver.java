...
/**
 * hadoop jar /home/lib/wordnet-repo-0.5.2.jar -libjars ${LIBJARS} /tmp/wordnetxml/wordnet-ref.xml /tmp/wordnetxml/wordnet-synsets.xml /tmp/wordnetxml/wordnet-words.xml /tmp/wordnetxml/wordnet-links.xml   
 */
public class CassandraMultiImportDriver {
	private static Log log = LogFactory.getLog(CassandraMultiImportDriver.class);

	/*
	 * Prints usage without error message
	 */
	private static void printUsage() {
		System.err.println("Usage: " + CassandraMultiImportDriver.class.getSimpleName());
	}
	...
	public static void runJob(Configuration conf, String[] otherArgs) throws IOException {
		...
		for (String input : otherArgs) {
			Job job = new Job(conf, CassandraMultiImportDriver.class.getSimpleName());
			FileInputFormat.setInputPaths(job, input);
			job.setJarByClass(CassandraMultiImportDriver.class);
			job.setMapperClass(CassandraImportMapper.class);				 
